...
local makeCheckbox
do
    local function checkboxGetValue(self) return ns.db[self.key] and true or false end
    local function checkboxSetChecked(self) self:SetChecked(self:GetValue()) end
    local function checkboxSetValue(self, checked)
        ns.db[self.key] = checked and true or false
...
local function makeItemButton(parent)
    local button = CreateFrame("BUTTON", "ItemButton"..counter, parent, "ItemButtonTemplate")
    counter = counter + 1
    -- classic
    if not button.SetItem then
        function button:SetItem(item)
            local itemInfos = GetItemInfoInstant(item)
            if itemInfos.itemID then
                self.itemID = itemInfos.itemID
                SetItemButtonTexture(self, itemInfos.icon)
                SetItemButtonQuality(self, itemInfos.quality)
            end
        end
...
function ns:SetupConfig()
    ...
    do
        ...
        demo:SetScript("OnShow", function()
            ...
            for _, itemID in ipairs({19019, 19364, 10328, 11122, 23192, 7997, 14047, 1642947, 388875}) do
        ...
        makeCheckboxList(frame, {
            ...
            {"missingcharacter", "Show missing gems/enchants in character frame only"},
            {"bound", ("Flag items that are %s (%s)"):format(ITEM_SOULBOUND, CreateAtlasMarkup(ns.soulboundAtlas, 0.7, 0, 0)), "Only on items you control; bags and character"},
